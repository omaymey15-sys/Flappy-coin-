...
class GameView(
    ...
    private val onGameOverCallback: (score: Int, coins: Int, distMeters: Int, elapsedMs: Long) -> Unit
) : SurfaceView(ctx), Runnable, SurfaceHolder.Callback {
    ...
    private object GamePreferences {
    ...
    private var pipeGap: Float = 0f
    private var pipeHorizontalSpacing: Float = 0f
    private var pipeW: Float = 0f
    private var pipeH: Float = 0f
    private var minGapCenter: Float = 0f
    private var maxGapCenter: Float = 0f
    ...
    private val scorePaint = Paint().apply {
        color = Color.WHITE; textSize = 120f; isFakeBoldText = true; textAlign = Paint.Align.CENTER
    ...
    private val coinEmojiPaint = Paint().apply {
        textSize = 70f; textAlign = Paint.Align.CENTER
    }
    private val particlePaint = Paint().apply { isAntiAlias = true }
    private val cardPaint = Paint().apply { color = Color.argb(180, 44, 44, 44) }
    ...
    private var pipeBmp: Bitmap
    private var pipeTopBmp: Bitmap
    private var spikeBmp: Bitmap? = null
    private val birdFrames = ArrayList<Bitmap>()

    // motion & physics
    private var scrollSpeed = 6.5f
    private var scrollAcceleration = 0.01f
    private var gravity = 0.6f
    private var jumpImpulse = -14f
    ...
    init {
        ...
        sWing = soundPool.load(ctx, R.raw.wing, 1)
        sPoint = soundPool.load(ctx, R.raw.point, 1)
        sHit = soundPool.load(ctx, R.raw.hit, 1)
        sDie = soundPool.load(ctx, R.raw.die, 1)
    ...
    override fun run() {
        while (playing) {
            ...
            try { Thread.sleep(Constants.FRAME_TIME_MS) } catch (_: Exception) {}
        }
    }
    ...
    private fun update(deltaMs: Long) {
        ...
        if (currentState != GameState.GAME_OVER) {
            // Scroll base and bg
            baseX1 -= scrollSpeed * f; baseX2 -= scrollSpeed * f
            if (baseX1 + screenW <= 0) baseX1 = baseX2 + screenW
            if (baseX2 + screenW <= 0) baseX2 = baseX1 + screenW

            bgX1 -= (scrollSpeed / 4) * f; bgX2 -= (scrollSpeed / 4) * f
            ...
            if (System.currentTimeMillis() - lastCoinTime > Constants.COMBO_DURATION_MS) {
                comboCounter = 0; comboMultiplier = 1f
            }
        }
        ...
        when (currentState) {
            ...
            GameState.PLAYING -> {
                ...
                if (birdY < 0) { birdY = 0f; triggerGameOver() }
                ...
                while (itObstacle.hasNext()) {
                    ...
                    if (!o.passed && o.x + pipeW < birdX) {
                        ...
                        gapsCollected++
                        if (gapsCollected % gapsForDifficulty == 0) increaseDifficulty()
                    }

                    if (o.x + pipeW < 0) {
                        itObstacle.remove()
                        if (obstacles.isNotEmpty()) newObstacleX = obstacles.last().x + pipeHorizontalSpacing
                        else newObstacleX = screenW + pipeHorizontalSpacing
                    }
                }
    ...
    private fun triggerGameOver() {
        if (currentState != GameState.GAME_OVER) {
            ...
            onGameOverCallback(score, coinsCount, distance / 10, elapsedTimeMs)
        }
    }
    ...
    fun pause() {
        playing = false
        try { gameThread?.join() } catch (_: Exception) {}
    }
    ...
    override fun surfaceCreated(holder: SurfaceHolder) { /* nothing extra */ }
    override fun surfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) { /* nothing */ }
