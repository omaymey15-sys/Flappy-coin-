...
class GameView(
    ...
    // Verrou de synchronisation pour éviter les accès concurrents
        ...
        synchronized(gameLock) {
            for (o in obstacles) {
                when (o.type) {
                    ...
                    ObstacleType.SPIKE -> {
                        spikeBmp?.let {
                            canvas.drawBitmap(it, o.x + pipeW / 2 - it.width / 2, o.centerY - pipeGap / 2 - 50, paint)
                            canvas.drawBitmap(it, o.x + pipeW / 2 - it.width / 2, o.centerY + pipeGap / 2 + 30, paint)
                        }
                    }
    ...
    private fun drawPlayingHUD(canvas: Canvas) {
        ...
        if (comboCounter > 1) {
            canvas.drawText("x${String.format("%.1f", comboMultiplier)}", screenW / 2f, screenH * 0.15f, hudPaint)
        }
    }
