...
class GameView(
    ...
    init {
        ...
        // Audio - UNIQUEMENT les sons du jeu
    ...
    private fun update(deltaMs: Long) {
        ...
        when (currentState) {
            ...
            GameState.PLAYING -> {
                ...
                birdVel += gravity * f
                // Correction: utilisation de coerceIn au lieu de min/max
    ...
    fun resume() {
        // Éviter de créer un nouveau thread si l'ancien existe encore
        if (gameThread != null && gameThread?.isAlive == true) return
        playing = true
